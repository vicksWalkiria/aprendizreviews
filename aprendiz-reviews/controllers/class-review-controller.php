...
class Aprendiz_Reviews_Review_Controller {
    ...
    public function display_list() {
        ...
        if (isset($_POST['validar_resenas']) && !empty($_POST['resenas_validar'])) {
            ...
            if ($result !== false) {
                $message = sprintf(_n('%d reseña validada.', '%d reseñas validadas.', count($ids), 'aprendiz-reviews'), count($ids));
                $message_type = 'success';
            } else {
                $message = __('Error al validar reseñas.', 'aprendiz-reviews');
                $message_type = 'error';
            }
        }
        ...
        if (isset($_GET['eliminar_id'])) {
            ...
            if ($result !== false) {
                $message = __('Reseña eliminada.', 'aprendiz-reviews');
                $message_type = 'success';
            } else {
                $message = __('Error al eliminar reseña.', 'aprendiz-reviews');
                $message_type = 'error';
            }
        }
    ...
    private function handle_form_submission() {
        ...
        if (!empty($_POST['editar_id'])) {
            ...
            return array(
                ...
                'message' => $result !== false ? __('Reseña actualizada.', 'aprendiz-reviews') : __('Error al actualizar reseña.', 'aprendiz-reviews'),
            ...
            return array(
                'success' => $result !== false,
                'message' => $result !== false ? __('Reseña guardada.', 'aprendiz-reviews') : __('Error al guardar reseña.', 'aprendiz-reviews'),
                'type' => $result !== false ? 'success' : 'error'
            );
        }
    }
